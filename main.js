...
// ═══════════════════════════════════════════════════════════════════════════
// Network Discovery System (SIGINT Protocol)
// ═══════════════════════════════════════════════════════════════════════════
const NETWORK_CONFIG = {
    PORT: 47823,                    // Uncommon port to avoid conflicts
    BROADCAST_INTERVAL_MS: 5000,    // Broadcast presence every 5 seconds
    STALE_THRESHOLD_MS: 15000,      // Consider node stale after 15 seconds
    CLEANUP_INTERVAL_MS: 10000,     // Clean up stale nodes every 10 seconds
    PROTOCOL_VERSION: 'SIGINT-1.0', // Protocol identifier
};
...
let discoveredNodes = new Map(); // Map of nodeId -> node data
let localNodeId = null;

function generateNodeId() {
    // Generate a unique node ID based on machine + random
...
function startNetworkDiscovery() {
    ...
    try {
        ...
        udpSocket.on('listening', () => {
            ...
            broadcastPresence(); // Immediate first broadcast
...
function broadcastPresence() {
    ...
    const rank = getRank(level)?.name || 'TRAINEE';
...
function calculateSignalStrength(remoteIp) {
    // Estimate signal strength based on IP similarity (same subnet = stronger)
    ...
    for (const local of localAddresses) {
        ...
        if (localParts[0] === remoteParts[0] && localParts[1] === remoteParts[1]) {
            if (localParts[2] === remoteParts[2]) {
                strength = 'STRONG'; // Same /24 subnet
            } else {
                strength = 'MODERATE'; // Same /16
            }
        }
    }
...
function createWindow() {
    ...
    const winWidth = 240;
    const winHeight = 292; // Extra height for glow at top
...
${recentContext ? `RECENT CONVO:\n${recentContext}` : ''}`;
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // Network Discovery IPC (SIGINT Protocol)
    // ═══════════════════════════════════════════════════════════════════════
